...
def _assign_components_to_devices(
    module_sizes: Dict[str, float], device_memory: Dict[str, float], device_mapping_strategy: str = "balanced", gpu_only: bool = False
):
    ...
    for component in module_sizes:
        ...
        if component_memory > curr_device_memory:
            if gpu_only:
                assigned = False
                for gpu_id in sorted(device_memory.keys(), key=lambda x: device_memory[x], reverse=True):
                    if device_memory[gpu_id] >= component_memory:
                        if gpu_id not in device_id_component_mapping:
                            device_id_component_mapping[gpu_id] = [component]
                        else:
                            device_id_component_mapping[gpu_id].append(component)
                        device_memory[gpu_id] -= component_memory
                        assigned = True
                        break
                
                if not assigned:
                    best_gpu = max(device_memory.keys(), key=lambda x: device_memory[x])
                    if best_gpu not in device_id_component_mapping:
                        device_id_component_mapping[best_gpu] = [component]
                    else:
                        device_id_component_mapping[best_gpu].append(component)
                    device_memory[best_gpu] -= component_memory
                
                current_device_index += 1
            else:
                if "cpu" not in device_id_component_mapping:
                    device_id_component_mapping["cpu"] = [component]
                else:
                    device_id_component_mapping["cpu"].append(component)
        else:
...
def _get_final_device_map(device_map, pipeline_class, passed_class_obj, init_dict, library, max_memory, gpu_only=False, **kwargs):
    from diffusers import pipelines
    ...
    if len(max_memory) > 0:
        device_id_component_mapping = _assign_components_to_devices(
            module_sizes, max_memory, device_mapping_strategy=device_map, gpu_only=gpu_only
        )
